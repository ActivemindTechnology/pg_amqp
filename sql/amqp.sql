...
create function amqp.publish(integer, varchar, varchar, varchar, varchar, boolean)
returns boolean as 'pg_amqp.so', 'pg_amqp_publish'
language C immutable;

comment on function amqp.publish(integer, varchar, varchar, varchar, varchar, boolean) is
'Publishes a message (broker_id, exchange, routing_key, message, content_type, persistent).  The message will only
...
create function amqp.autonomous_publish(integer, varchar, varchar, varchar, varchar, boolean)
returns boolean as 'pg_amqp.so', 'pg_amqp_autonomous_publish'
language C immutable;

comment on function amqp.autonomous_publish(integer, varchar, varchar, varchar, varchar, boolean) is
